...
//封装一个运动函数
//元素	运动属性 速度   目标点	回调函数
function myMove(obj, attr, speed, target, callback) {
	// var timer =null;
	//后续将定时器  挂载到当前运动的物体的自定义属性timer上面
	// obj.timer = null;
	//开启定时器之前将之前的定时器清除,保证页面上只有一个定时器
	clearInterval(obj.timer);
	// var dis = oBox.offsetLeft//将声明变量提到定时器外面,避免重复声明
	//通过调用getStyle函数获取任意样式
	var dis = parseFloat(getStyle(obj, attr));//获取的样式带单位,转化为数字类型

	//通过判断起始目标点的大小,来判断speed的正负   起始位置大于目标点位置则为负,反之为正
	speed = dis > target ? -speed : speed;
	obj.timer = setInterval(function () {
		dis += speed;
		//进行一次判断,如果超过目标点,则拉回目标点
		...
		if (dis == target) {
			...
			clearInterval(obj.timer);
			obj.timer = null;
			callback && callback();//使用回单函数
			//如果认识fn则执行fn(),如果不认识不报错
		}
	}, 30)
